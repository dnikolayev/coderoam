...
func runCodex(req request) (string, time.Duration, error) {
	...
	args = appendWorkspaceWriteAddDirs(args, sandbox, resumeMode, sessionID)
	...
	req = transcribeAudioAttachments(ctx, req, "CODEX_RUNNER")
...
func appendEnvArgs(args []string) []string {
	if approvalPolicy := strings.TrimSpace(os.Getenv("CODEX_RUNNER_APPROVAL_POLICY")); approvalPolicy != "" {
...
func appendWorkspaceWriteAddDirs(args []string, sandbox, resumeMode, sessionID string) []string {
