...
Each Claude session must use its own clearly named WhatsApp group, alias, and
session id, such as `claude-session` for a group named "Claude Session". If a
new lane is needed, create a new group with `coderoam active start --name
"Claude Session" --alias <session-id> --session-id <session-id> --yes` instead
